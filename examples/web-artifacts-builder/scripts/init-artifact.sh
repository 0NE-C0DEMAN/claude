...
# Need either the pre-extracted directory or the tarball
if [ ! -d "$COMPONENTS_DIR" ] && [ ! -f "$COMPONENTS_TARBALL" ]; then
  echo "❌ Error: shadcn components not found in script directory"
  echo "   Expected: $COMPONENTS_DIR/ (directory) or $COMPONENTS_TARBALL"
  exit 1
fi
...
# Copy or extract shadcn components into src/
echo "📦 Installing shadcn/ui components..."
if [ -d "$COMPONENTS_DIR" ]; then
  cp -R "$COMPONENTS_DIR/." src/
else
